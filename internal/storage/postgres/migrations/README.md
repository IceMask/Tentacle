...
```bash
psql "$DATABASE_URL" -c '\dt sessions traces plan_events artifacts audit_logs'
```

You can also verify the expected indexes:

```bash
psql "$DATABASE_URL" -c '\di idx_sessions_* idx_traces_* idx_plan_events_* idx_artifacts_* idx_audit_logs_*'
```
