# MCP Mobile Worker — 系统设计说明（v4 综合版）

> 本设计文档对齐《requirements.v4.md》，在 Module I 的实现蓝图基础上，扩展 Module II 与 Module III 的组件职责、接口、数据流与部署建议，供架构评审与各子团队实施参考。
...
### 2.1 Gateway
- **main.go**：配置加载 → Telemetry 初始化 → gRPC 客户端与缓存初始化 → 构建 JSON-RPC Handler (`internal/gateway/jsonrpc`), REST Router (`internal/gateway/rest`), WebSocket Hub (`internal/gateway/websocket`) → 注册中间件（Auth→RateLimit→Logging）→ TLS/HTTP Serve。
...
- **WebSocket Hub**：`publishCh` 异步广播，队列长度 256 drop-oldest；慢连接检测+断开；指标 `websocket_drops_total`、`ws_queue_len_histogram`，语义为 at-most-once，消费者需结合 A2A 事件拉取补齐。

### 2.2 Orchestrator
- **service.go**：
  - `StartSession/ExecutePlan/CancelPlan/HealthCheck/...` 将 DAO、WorkerRegistry、EventsPublisher、S3 Presign 组合。
  - `HealthCheck` 读取 Postgres、Redis ping 与 Worker 心跳延迟生成 `issues[]`（类型/严重度/建议）。
- **dispatcher.go**：基于 Redis Streams，按 `tenant/project` 分片；`XAUTOCLAIM` 做故障接管；支持优先级权重。
- **worker_registry.go**：维护 Redis Hash 租约；`Assign` 考虑标签/负载；`StartMonitor` 处理超时与降权。
...
- **heartbeat.go**：10s 周期上报状态；连续失败仅告警，3 次后触发 orchestrator 降权。

### 2.4 数据层
- **Postgres**：表 `sessions`, `traces`, `plan_events`, `artifacts`, `audit_logs`。使用行级锁/乐观锁，所有查询命中索引；迁移脚本放 `internal/storage/postgres/migrations`。
- **Redis**：
  - `Streams` 用于排队；
...
- Auth 中间件：优先 HMAC→OIDC→PAT；失败 401/403；审计写入 Postgres。
